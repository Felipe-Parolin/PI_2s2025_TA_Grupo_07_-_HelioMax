...
if (!isset($_SESSION['usuario_id'])) {
    ...
    <head>
        ...
        <title>
        <?php 
        if ($modo_reset) echo 'Redefinir Senha';
        elseif ($modo_recuperacao) echo 'Recuperar Senha';
        else echo 'Login'; 
        ?> 
        - HelioMax Admin</title>
    ...
    <body class="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
        <div class="w-full max-w-md">
            <div class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-8 border border-cyan-500/20">
                ...
                <?php 
                if ($modo_reset) echo 'Defina sua nova senha';
                elseif ($modo_recuperacao) echo 'Informe seu e-mail para recuperar a senha';
                else echo 'Faça login para continuar';
                ?>
                </p>
                ...
                <form method="POST" action="">
                    ...
                    <button type="submit" class="w-full bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 hover:scale-105 shadow-lg shadow-cyan-500/30">
                        Redefinir Senha
                    </button>
                    
                    <p class="text-center mt-4">
                        <a href="dashADM.php" class="text-gray-400 text-sm hover:text-cyan-400 transition-colors">Voltar para o Login</a>
                    </p>
                </form>
                ...
                <form method="POST" action="">
                    ...
                    <p class="text-center mt-4">
                        <a href="?action=recuperar_senha" class="text-gray-400 text-sm hover:text-cyan-400 transition-colors">Esqueceu a senha?</a>
                    </p>
                </form>
...
<body class="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        ...
        <div class="flex items-center gap-4">
          ...
          <button onclick="abrirModal('modalPerfil')" class="p-2 bg-slate-800/50 hover:bg-slate-700/50 rounded-lg border border-cyan-500/20 hover:border-cyan-500/40 transition-all">
            <i data-lucide="settings" class="w-5 h-5 text-cyan-400"></i>
          </button>
          
          <a href="?logout=1" class="p-2 bg-red-500/20 hover:bg-red-500/30 rounded-lg border border-red-500/30 hover:border-red-500/50 transition-all">
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <?php if ($mensagem): ?>
    <div class="mb-6 p-4 <?php echo $tipo_mensagem === 'sucesso' ? 'bg-green-500/20 border-green-500/30' : 'bg-red-500/20 border-red-500/30'; ?> border rounded-xl flex items-center gap-3">
      <i data-lucide="<?php echo $tipo_mensagem === 'sucesso' ? 'check-circle' : 'alert-circle'; ?>" class="w-5 h-5 <?php echo $tipo_mensagem === 'sucesso' ? 'text-green-400' : 'text-red-400'; ?>"></i>
      <p class="<?php echo $tipo_mensagem === 'sucesso' ? 'text-green-400' : 'text-red-400'; ?>"><?php echo $mensagem; ?></p>
    </div>
    <?php endif; ?>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <div class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-6 border border-cyan-500/20 hover:border-cyan-500/40 transition-all duration-300 hover:scale-105">
      ...
      <div class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-6 border border-cyan-500/20 hover:border-cyan-500/40 transition-all duration-300 hover:scale-105">
        <div class="flex items-center justify-between mb-4">
          <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-orange-500 to-yellow-500 flex items-center justify-center">
    ...
    <div class="mb-8">
      <button onclick="abrirModal('modalCriarPonto')" class="w-full sm:w-auto bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-bold py-4 px-8 rounded-2xl flex items-center justify-center gap-3 shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/50 transition-all duration-300 hover:scale-105">
        <i data-lucide="plus" class="w-6 h-6"></i>
        <span class="text-lg">Cadastrar Novo Ponto de Recarga</span>
      </button>
    </div>

    <div class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-6 border border-cyan-500/20 mb-6">
      <form method="GET" action="" class="flex flex-col sm:flex-row gap-4">
        <div class="flex-1 relative">
          ...
          <input type="text" name="busca" value="<?php echo htmlspecialchars($busca); ?>" placeholder="Buscar por endereço..."
        ...
        <select name="status" class="px-6 py-3 bg-slate-900/50 border border-cyan-500/20 rounded-xl text-white focus:outline-none focus:border-cyan-500/50 transition-colors cursor-pointer">
          ...
          <option value="Manutenção" <?php echo $status_filtro === 'Manutenção' ? 'selected' : ''; ?>>Em Manutenção</option>
        </select>
        <button type="submit" class="px-6 py-3 bg-cyan-500 hover:bg-cyan-600 text-white rounded-xl font-semibold transition-colors">
          Filtrar
        </button>
      </form>
    </div>

    <div class="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl border border-cyan-500/20 overflow-hidden">
        ...
        <table class="w-full">
          <thead class="bg-slate-900/50">
            <tr>
              ...
              <th class="px-6 py-4 text-center text-xs font-semibold text-cyan-400 uppercase tracking-wider hidden sm:table-cell">Valor kWh</th>
            ...
            <tr class="hover:bg-slate-800/30 transition-colors">
              ...
              <td class="px-6 py-4">
                <div class="flex items-start gap-3">
                  <div class="w-10 h-10 bg-gradient-to-br from-cyan-500/20 to-blue-500/20 rounded-lg flex items-center justify-center flex-shrink-0 border border-cyan-500/30">
                    <i data-lucide="map-pin" class="w-5 h-5 text-cyan-400"></i>
                  </div>
                  <div>
                    <p class="font-semibold"><?php echo htmlspecialchars($ponto['LOGRADOURO'] ?? 'Não informado'); ?></p>
              ...
              <td class="px-6 py-4 text-center hidden sm:table-cell">
                <span class="inline-flex items-center gap-1 px-3 py-1 bg-blue-500/20 text-blue-400 rounded-full text-sm font-semibold border border-blue-500/30">
  ...
  <div id="modalCriarPonto" class="modal fixed inset-0 bg-black/70 backdrop-blur-sm items-center justify-center z-50 p-4">
    <div class="bg-gradient-to-br from-slate-800/95 to-slate-900/95 backdrop-blur-xl rounded-2xl border border-cyan-500/20 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <form method="POST" action="">
        ...
        <input type="hidden" name="id_ponto" value="0"> <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="logradouro" id="logradouro_criar" required placeholder="Ex: Av. Brigadeiro Faria Lima"
                    class="w-full px-4 py-3 bg-slate-900/50 border border-cyan-500/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500/50 transition-colors">
                </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                <button type="submit" class="flex-1 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 hover:scale-105 shadow-lg shadow-cyan-500/30">
                    <i data-lucide="save" class="w-5 h-5 inline-block mr-2"></i>
                    Cadastrar Ponto
                </button>
                
                <button type="button" onclick="fecharModal('modalCriarPonto')" class="flex-1 bg-slate-700/50 hover:bg-slate-700 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 border border-slate-600">
...
<div id="modalEditarPonto" class="modal fixed inset-0 bg-black/70 backdrop-blur-sm items-center justify-center z-50 p-4">
    <div class="bg-gradient-to-br from-slate-800/95 to-slate-900/95 backdrop-blur-xl rounded-2xl border border-cyan-500/20 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
      <div class="p-6 border-b border-cyan-500/20 flex items-center justify-between sticky top-0 bg-slate-900/90">
        <h2 class="text-2xl font-bold text-white flex items-center gap-2">
          <i data-lucide="map-pin" class="w-6 h-6 text-cyan-400"></i>
        ...
        <button onclick="fecharModal('modalEditarPonto')" class="p-2 hover:bg-slate-700/50 rounded-lg transition-colors">
          <i data-lucide="x" class="w-6 h-6 text-gray-400"></i>
        </button>
      </div>
      ...
      <form method="POST" action="">
        <input type="hidden" name="action" value="salvar_ponto">
        <input type="hidden" name="id_ponto" id="id_ponto_editar" value=""> <div class="p-6">
            <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                <i data-lucide="map" class="w-5 h-5 text-cyan-400"></i>
                Endereço do Ponto
            </h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div class="md:col-span-2">
                    <label class="block text-gray-400 text-sm font-semibold mb-2">Logradouro *</label>
                    <input type="text" name="logradouro" id="logradouro_editar" required placeholder="Ex: Av. Brigadeiro Faria Lima"
                    class="w-full px-4 py-3 bg-slate-900/50 border border-cyan-500/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500/50 transition-colors">
                </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <button type="button" onclick="fecharModal('modalEditarPonto')" class="flex-1 bg-slate-700/50 hover:bg-slate-700 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 border border-slate-600">
                    <i data-lucide="x" class="w-5 h-5 inline-block mr-2"></i>
                    Cancelar
                </button>
            </div>
    ...
    <div class="bg-gradient-to-br from-slate-800/95 to-slate-900/95 backdrop-blur-xl rounded-2xl border border-cyan-500/20 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <div class="border-b border-cyan-500/20 px-6">
        <div class="flex gap-4">
          <button onclick="mudarTab('dadosPessoais')" id="tabDadosPessoais" class="px-4 py-3 text-white font-semibold border-b-2 border-cyan-500 transition-colors">
            Dados Pessoais
          </button>
          <button onclick="mudarTab('alterarSenha')" id="tabAlterarSenha" class="px-4 py-3 text-gray-400 font-semibold border-b-2 border-transparent hover:text-white transition-colors">
            Alterar Senha
          </button>
        </div>
      </div>
        ...
        <form method="POST" action="">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              ...
              <input type="text" name="cpf" required value="<?php echo htmlspecialchars($usuario['CPF']); ?>" maxlength="11"
                oninput="this.value = this.value.replace(/\D/g, '')"
                inputmode="numeric"
                class="w-full px-4 py-3 bg-slate-900/50 border border-cyan-500/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500/50 transition-colors">
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="md:col-span-2">
              <label class="block text-gray-400 text-sm font-semibold mb-2">Logradouro</label>
              <input type="text" name="logradouro_perfil" value="<?php echo htmlspecialchars($usuario['LOGRADOURO'] ?? ''); ?>" placeholder="Ex: Av. Paulista"
                class="w-full px-4 py-3 bg-slate-900/50 border border-cyan-500/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500/50 transition-colors">
            </div>

            <div>
              <label class="block text-gray-400 text-sm font-semibold mb-2">Número (Apenas Números)</label>
              <input type="text" name="numero_residencia" value="<?php echo htmlspecialchars($usuario['NUMERO_RESIDENCIA'] ?? ''); ?>"
                oninput="this.value = this.value.replace(/\D/g, '')"
                inputmode="numeric"
                class="w-full px-4 py-3 bg-slate-900/50 border border-cyan-500/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500/50 transition-colors">
            </div>
            ...
            <div>
              <label class="block text-gray-400 text-sm font-semibold mb-2">Complemento</label>
              <input type="text" name="complemento" value="<?php echo htmlspecialchars($usuario['COMPLEMENTO_ENDERENCO'] ?? ''); ?>"
                class="w-full px-4 py-3 bg-slate-900/50 border border-cyan-500/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500/50 transition-colors">
            </div>

            <div>
              <label class="block text-gray-400 text-sm font-semibold mb-2">Bairro</label>
              <input type="text" name="bairro_perfil" value="<?php echo htmlspecialchars($usuario['bairro'] ?? ''); ?>" placeholder="Ex: Bela Vista"
                class="w-full px-4 py-3 bg-slate-900/50 border border-cyan-500/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500/50 transition-colors">
            </div>

            <div>
              <label class="block text-gray-400 text-sm font-semibold mb-2">Cidade</label>
              <input type="text" name="cidade_perfil" value="<?php echo htmlspecialchars($usuario['cidade'] ?? ''); ?>" placeholder="Ex: São Paulo"
                class="w-full px-4 py-3 bg-slate-900/50 border border-cyan-500/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500/50 transition-colors">
            </div>

            <div>
              <label class="block text-gray-400 text-sm font-semibold mb-2">UF</label>
              <input type="text" name="uf_perfil" value="<?php echo htmlspecialchars($usuario['UF'] ?? ''); ?>" placeholder="Ex: SP" maxlength="2"
                class="w-full px-4 py-3 bg-slate-900/50 border border-cyan-500/20 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500/50 transition-colors uppercase">
            </div>
          </div>
      ...
      <div id="contentAlterarSenha" class="p-6 hidden">
        <form method="POST" action="">
          ...
          <button type="submit" class="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 hover:scale-105 shadow-lg shadow-purple-500/30">
